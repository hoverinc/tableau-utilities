...
Apply a config to a tdsx

```commandline
tableau_utilities --name "Datasource Name" --file_path "{path}.tds" --debugging_logs apply_configs --column_config "{path}.json" --calculated_column_config "{path}.json"
```
